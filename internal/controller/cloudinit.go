...
// isK0sBootstrap returns true if the cloud-init config was generated by k0smotron
// (i.e., runcmd contains k0s install/start commands rather than kubeadm commands).
func isK0sBootstrap(config CloudInitConfig) bool {
	for _, cmd := range config.RunCmd {
		...
		if strings.Contains(cmdStr, "k0s install") || strings.Contains(cmdStr, "k0s start") {
			return true
		}
	}
	return false
...
// 2. For kubeadm bootstrap: installs Kubernetes prerequisites (containerd, kubeadm, kubelet)
//    For k0s bootstrap: skips the above (k0s bundles its own runtime)
...
func CloudInitToScript(cloudInitData string, kubeVersion string, machineName string) (string, error) {
	...
	if k0s {
		// k0s bundles its own containerd and does not use kubelet/kubeadm.
		// Installing those would conflict with k0s's internal runtime.
		// Just ensure curl and basic tools are available.
		script.WriteString("# k0s bootstrap: ensure curl is available\n")
		script.WriteString("apt-get update -q && apt-get install -y -q curl || true\n\n")
	} else {
		// Set Kubernetes version for the kubeadm preinstall script
		if kubeVersion != "" {
	...
	for _, f := range config.WriteFiles {
		...
		// Write content — if base64-encoded, decode via pipeline to avoid
		// heredoc issues with binary data (e.g. k0smotron join tokens).
		enc := strings.ToLower(f.Encoding)
		if enc == "b64" || enc == "base64" {
			script.WriteString(fmt.Sprintf("echo '%s' | base64 -d > '%s'\n", f.Content, f.Path))
		} else {
